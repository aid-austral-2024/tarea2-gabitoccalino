...
#Por la naturaleza de los datos, se multiplica el IQR por 3.5 para eliminar sólo los valores atìpicos extremos.
...
Adicionalmente, se generó un gráfico de dispersión para una primera y rápida visualización de los datos. Los colores corresponden a las dos poblaciones.
...
ggplot(data = TDC, aes(x = TIME_STAMP, y = VALUE, color = GRUPO)) +
  geom_point(size = 3, alpha = 0.8) +  # Puntos de dispersión
  ...
  labs(title = "Gráfico de Dispersión de la Variable VALUE",
       x = "Fecha y Hora",
       y = "Value",
       color = "Grupo") +
  scale_color_manual(values = c("A" = "#7EC0EE", "B" = "#90EE90")) +  # Colores por grupo
  theme_minimal(base_size = 14) +  # Tema minimalista
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5),  # Título centrado y en negrita
    plot.subtitle = element_text(hjust = 0.5, margin = margin(b = 10)),  # Espaciado del subtítulo
